...
pub struct CliTransmitItem {
    pub payload: Vec<u8>,
}

pub struct Cli {
    pub send_queue: Arc<MpmcQueue<CliTransmitItem>>,
    pub recv_queue: Arc<MpmcQueue<CliTransmitItem>>,
...
//srv_recive_data
//if new_conn assign remote_ip-tun_ip and send package to tun
//other send package to tun

//srv_transmit_data
//find remote ip from assinged by tun_ip and send package to tun
//if remote ip not finded - drop

impl Cli {
    const MAX_MTU: usize = 1500;

    pub fn new(cfg: CliConfig) -> Result<Self, LokiError> {
        ...
        if let Some(err) = device
            .connect(SocketAddrV4::new(
                Ipv4Addr::from_octets(cfg.ipv4_host),
                cfg.ipv4_port,
            ))
            .err()
        {
    ...
    pub fn send(&self, i: CliTransmitItem) -> Result<Result<(), CliTransmitItem>, LokiError> {
        if self.is_running() {
    ...
    pub fn receive(&self) -> Result<Option<CliTransmitItem>, LokiError> {
        if self.is_running() {
    ...
    fn background_loop(
        send: Weak<MpmcQueue<CliTransmitItem>>,
        recv: Weak<MpmcQueue<CliTransmitItem>>,
        device: UdpSocket,
        ...
        loop {
            ...
            if !recv.is_full() {
                ...
                match device.recv(&mut raw_buff) {
                    Ok(len) if len > 0 => {
                        let mut payload = Vec::new();
                        payload.reserve(len);
                        payload.extend_from_slice(&raw_buff[..len]);
                        some_received = true;
                        recv.push_blocking(CliTransmitItem { payload });
                    }
